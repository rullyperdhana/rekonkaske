<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4 border-b-2 border-primary pb-4">
            ...
            <div>
                <h2 class="text-headline-lg font-headline-lg text-on-surface">Edit Rekening</h2>
                <p class="text-body-md font-body-md text-on-surface-variant mt-1">Ubah data rekening bank dan relasinya ke SKPD</p>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden p-6">
            ...
            <form action="{{ route('rekening.update', $rekening->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="skpd_id" required class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        ...
                        @foreach($skpds as $skpd)
                            <option value="{{ $skpd->id }}" {{ old('skpd_id', $rekening->skpd_id) == $skpd->id ? 'selected' : '' }}>{{ $skpd->nama }}</option>
                        @endforeach
                    </select>
                </div>
                @endif
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $rekening->nama) }}" required class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Misal: Kas Daerah">
                </div>
                
                <div>
                    <label class="block text-label-sm font-label-sm text-on-surface font-semibold mb-1">Nomor Rekening</label>
                    <input type="text" name="nomor" value="{{ old('nomor', $rekening->nomor) }}" required class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Misal: 001.03.01.XXXXX">
                </div>

                <div>
                    <label class="block text-label-sm font-label-sm text-on-surface font-semibold mb-1">Bank</label>
                    <input type="text" name="bank" value="{{ old('bank', $rekening->bank) }}" required class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Misal: Bank Kalsel">
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        ...
                        <option value="0" {{ old('status', $rekening->status) == '0' ? 'selected' : '' }}>Non-Aktif</option>
                    </select>
                </div>
                ...
                <div class="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" class="bg-primary hover:bg-primary-fixed-variant text-on-primary px-5 py-2.5 rounded-lg text-label-sm font-label-sm font-semibold shadow-sm transition-colors">Perbarui Rekening</button>
                </div>
            </form>
        </div>
    </div>
