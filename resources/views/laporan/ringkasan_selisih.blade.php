...
<style>
    /* Make this specific report page full width instead of bounded by container-max */
    #appMain {
...
<div class="bg-surface rounded-xl shadow-sm border border-outline-variant p-6 mb-8">
    <form action="{{ route('laporan.ringkasan-selisih') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('laporan.ringkasan-selisih.excel', request()->all()) }}" target="_blank" class="h-11 px-6 bg-green-600 text-white hover:bg-green-700 rounded-lg flex items-center gap-2 font-label-md transition-colors shadow-sm whitespace-nowrap">
                <span class="material-symbols-outlined" data-weight="fill">table_view</span>
                Ekspor Excel
            </a>
            <a href="{{ route('laporan.ringkasan-selisih.pdf', request()->all()) }}" target="_blank" class="h-11 px-6 bg-tertiary text-on-tertiary hover:bg-tertiary/90 rounded-lg flex items-center gap-2 font-label-md transition-colors shadow-sm whitespace-nowrap">
