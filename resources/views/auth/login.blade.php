...
<head>
    ...
    <title>Login - SiReKa (Sistem Rekonsiliasi Kas) Kabupaten Tapin</title>
...
<body class="flex flex-col justify-between min-h-screen relative">
    ...
    <header class="relative z-10 w-full px-4 sm:px-8 lg:px-12 py-3 sm:py-4 flex items-center justify-between">
        ...
        <a href="{{ route('landing') }}" class="inline-flex items-center gap-2.5 sm:gap-3 px-3.5 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/25 shadow-sm hover:bg-white/25 transition-all group">
            @if($logoLogin)
                <img src="{{ $logoLogin }}" alt="Logo Kabupaten Tapin" class="w-6 h-6 sm:w-7 sm:h-7 object-contain drop-shadow-xs">
            @else
                <span class="material-symbols-outlined text-white text-[22px] group-hover:text-blue-200 transition">account_balance</span>
            @endif
            <div class="text-left">
                <span class="block text-xs font-black text-white tracking-tight leading-tight group-hover:text-blue-200 transition">SiReKa</span>
                <span class="block text-[9.5px] sm:text-[10px] font-semibold text-blue-100/80 leading-none">Kabupaten Tapin</span>
            </div>
        </a>
                ...
                <div class="text-center mb-4">
                    ...
                    <div class="flex flex-col items-center justify-center gap-1.5 mb-2">
                        <div class="p-2.5 bg-slate-50 rounded-2xl shadow-2xs border border-slate-200 inline-flex items-center justify-center">
                            @if($logoLogin)
                                <img src="{{ $logoLogin }}" alt="Logo Kabupaten Tapin" class="w-11 h-11 object-contain">
                            @else
                                <span class="material-symbols-outlined text-[#00346f] text-[36px]">account_balance</span>
                            @endif
                        </div>
                        <div class="flex flex-col items-center">
                            ...
                            <h2 class="text-[11px] font-bold text-slate-600 mt-0.5">BKAD Kabupaten Tapin</h2>
                        </div>
                    </div>
    ...
    <footer class="relative z-10 py-3 text-center px-4 w-full">
        <div class="inline-block max-w-full px-4 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/20 shadow-2xs text-center">
            <p class="text-[10px] sm:text-[11px] font-semibold text-blue-100 leading-normal">
                &copy; {{ date('Y') }} Badan Keuangan dan Aset Daerah (BKAD) Pemerintah Kabupaten Tapin.
            </p>
        </div>
    </footer>
    ...
    <script>
        ...
        function updateClock() {
            ...
            const now = new Date();
            // Tapin / South Kalimantan operates in WITA (UTC+8)
