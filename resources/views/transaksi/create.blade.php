        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('input[data-target]').forEach(displayInput => {
                ...
                displayInput.addEventListener('input', function() {
                    ...
                    // Adjust cursor position relative to end
                    ...
                    fetch(`{{ route('transaksi.getSaldoAwal') }}?skpd_id=${skpdId}&rekening_id=${rekeningId}&periode_bulan=${periodeBulan || 1}&periode_tahun=${periodeTahun}`)
                        ...
                        .then(data => {
                            ...
                            if(data.bku_saldo_akhir > 0 || data.bank_saldo_akhir > 0) {
                                const bkuVal = parseFloat(data.bku_saldo_akhir) || 0;
                                const bankVal = parseFloat(data.bank_saldo_akhir) || 0;
                                bkuAwal.value = bkuVal.toFixed(2);
                                bankAwal.value = bankVal.toFixed(2);
                                // Update display inputs
                                document.getElementById('bku_saldo_awal_display').value = formatRupiahDisplay(bkuVal);
                                document.getElementById('bank_saldo_awal_display').value = formatRupiahDisplay(bankVal);
                                calculateBku();
                                calculateBank();
                            }
                        })
