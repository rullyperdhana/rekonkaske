    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4 border-b-[3px] border-primary pb-4">
            ...
            <div>
                ...
                <p class="text-body-md font-body-md text-on-surface-variant mt-1">Ubah data akun Admin atau Operator</p>
            </div>
        </div>
        ...
        <div class="bg-surface rounded border border-outline-variant shadow-sm overflow-hidden p-6">
            ...
            <form action="{{ route('user.update', $user->id) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>

                <div>
                    <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Password (Kosongkan jika tidak ingin mengubah)</label>
                    <input type="password" name="password" class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>

                <div>
                    <label class="block font-label-sm text-label-sm text-on-surface font-semibold mb-1">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
                ...
                <div>
                    ...
                    <select name="skpd_id" class="w-full h-10 px-3 rounded border border-outline-variant bg-surface text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                        ...
                        @foreach($skpds as $skpd)
                            <option value="{{ $skpd->id }}" {{ old('skpd_id', $user->skpd_id) == $skpd->id ? 'selected' : '' }}>{{ $skpd->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" class="bg-primary hover:bg-primary-container text-on-primary hover:text-on-primary-container px-5 py-2.5 rounded font-label-sm text-label-sm font-semibold shadow-sm transition-colors">Perbarui Pengguna</button>
                </div>
            </form>
        </div>
    </div>
