<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-6">
        <div class="flex items-center gap-4 border-b-2 border-primary pb-4">
            ...
            <div>
                <h2 class="text-headline-lg font-headline-lg text-on-surface">Buat Pengumuman Baru</h2>
                <p class="text-body-md font-body-md text-on-surface-variant mt-1">Pengumuman akan muncul di halaman dashboard semua pengguna.</p>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden p-6">
            ...
            <form action="{{ route('pengumuman.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" required class="w-full h-[40px] px-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Misal: Perpanjangan Waktu Rekonsiliasi">
                </div>
                
                <div>
                    <label class="block text-label-sm font-label-sm text-on-surface font-semibold mb-1">Isi Pengumuman</label>
                    <textarea name="isi" required rows="5" class="w-full p-3 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" placeholder="Tulis pesan pengumuman di sini...">{{ old('isi') }}</textarea>
                </div>

                <div class="flex items-center gap-3 bg-surface-container-low p-4 rounded-lg border border-outline-variant/50">
                    <input type="checkbox" name="is_aktif" value="1" id="is_aktif" class="w-5 h-5 text-primary bg-surface-container border-outline-variant rounded focus:ring-primary focus:ring-2" checked>
